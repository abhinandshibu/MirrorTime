...
import { doc, collection, setDoc, getDoc, getDocs, query, where, getFirestore } from "firebase/firestore";
...
export const getCategoryName = (categories, id) => {
    ...
    return group === undefined
        ...
        : id[1] === 0
            ? group.name
            : group.subs.get(id[1]).name;
}

export const getCategoryColour = (categories, id) => {
    const group = categories.get(id[0]);
    return group === undefined
        ? "fff"
        : id[1] === 0
            ? group.colour
            : group.subs.get(id[1]).colour;
}
